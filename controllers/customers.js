...
module.exports = {
  async postCustomer(req, res) {
    ...
    for (var i = 0; i < checkCustTime.length; i++) {
      if (
        checkCustTime[i].fromTime === req.body.fromTime &&
        checkCustTime[i].toTime === req.body.toTime &&
        req.body.toTime !== 'anytime' &&
        req.body.fromTime !== 'anytime'
      ) {
        req.flash(
          'error',
          'You already have a customer with that scheduled time.'
        );
        res.redirect('back');
        return;
      }
    ...
    <strong><a href="/customers/${customer._id}">${customer.firstName} ${
      customer.lastName
    }</a></strong>
  ...
  async putEditCustomer(req, res) {
    ...
    if (
      req.body.fromTime !== findCustomer.fromTime &&
      req.body.toTime !== findCustomer.toTime
    ) {
      ...
      for (var i = 0; i < checkCustTime.length; i++) {
        if (
          ...
          checkCustTime[i].toTime === req.body.toTime &&
          req.body.fromTime !== 'anytime' &&
          checkCustTime[i].phoneNumber !== req.body.phoneNumber
        ) {
          req.flash(
            'error',
            'You already have a customer with that scheduled time.'
    ...
    <strong><a href="/customers/${updateCustomer._id}">${
      updateCustomer.firstName
    } ${updateCustomer.lastName}</a></strong>
  ...
  async getFindAll(req, res) {
    ...
    if (foundCustomers.docs.length < 1) {
      req.flash('error', `No customer found containing "${req.query.search}"`);
      res.redirect('back');
      return;
    }
    res.render('customers/customers', {
        ...
        Promise.all(
          customers.map(customer => {
            let message = `Hello ${customer.firstName}, this is ${
              user.firstName
            } from Dodson Pest Control. This is a reminder of your appointment tomorrow. If you have any questions please contact me at ${
              user.phoneNumber
            } Have a great day!
      `;
          ...
          .then(() => {
            req.flash(
              'success',
              'Messages have been sent to all of your customers for the day!'
            );
            res.redirect('back');
          })
          .catch(err => {
  ...
  async postSMS(req, res) {
    ...
    if (customer.fromTime === customer.toTime) {
      ...
      customer.fromTime === 'anytime' &&
      customer.toTime !== 'anytime'
    ) {
      let message = `Hello ${customer.firstName}, this is ${
        user.firstName
      } from Dodson Pest Control. This is a reminder of your appointment tomorrow I will be there no later than ${
        customer.toTime
      ...
      let message = `Hello ${customer.firstName}, this is ${
        user.firstName
      } from Dodson Pest Control. This is a reminder of your appointment from ${
        customer.fromTime
      } - ${
        customer.toTime
